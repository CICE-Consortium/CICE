...
module CICE_RunMod
  ...
  private :: coupling_prep
  private :: sfcflux_to_ocn
...
!=======================================================================
  ...
  subroutine coupling_prep (iblk)
    ...
    use ice_flux         , only: albpnd, albcnt, apeff_ai, coszen, fpond, fresh, l_mpond_fresh 
    use ice_flux         , only: alvdf_ai, alidf_ai, alvdr_ai, alidr_ai, fhocn_ai 
    use ice_flux         , only: fresh_ai, fsalt_ai, fsalt 
    use ice_flux         , only: fswthru_ai, fhocn, fswthru, scale_factor 
    use ice_flux         , only: fswthruvdr, fswthruvdf, fswthruidr, fswthruidf 
    use ice_flux         , only: swvdr, swidr, swvdf, swidf, Tf, Tair, Qa, strairxT, strairyt 
    use ice_flux         , only: fsens, flat, fswabs, flwout, evap, Tref, Qref, Uref, faero_ocn 
    use ice_flux         , only: fsurfn_f, flatn_f, scale_fluxes, frzmlt_init, frzmlt, wind 
       ...
       do j = 1, ny_block
          do i = 1, nx_block
             if (tmask(i,j) .and. aice(i,j) <= puny) then
                fhocn(i,j) = fhocn(i,j) + fsurfn_f(i,j,n) + flatn_f(i,j,n)
                fresh(i,j) = fresh(i,j) + flatn_f(i,j,n) * rLsub
             endif
          enddo   ! i
       enddo   ! j
    enddo      ! n
