...
*evp\_prep* in order to reduce the dynamics components work on a global
...
Two additional masks are created for the users convenience: `lmask\_n`
...
computed for diagnostic purposes and included here for the users
convenience.
...
example, the column timer (5) includes the timings from 610, and
...
users convenience. These log files were created using MPI on 4
...
combinations of updated decompositions and masked haloes, in CESMs
...
the default CICE v5 configuration (7 layers). In addition, CICEs
...
variables may overfill MPIs buffers, causing the code to slow down
